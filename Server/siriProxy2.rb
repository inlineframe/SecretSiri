...
class SiriProxyConnection < EventMachine::Connection
	...
	def get_speechId
	    begin
		File.open("../keys/speechId", "r") {|file| self.speechId = file.read}
	...
	def get_assistantId
	    begin
		File.open("../keys/assistantId", "r") {|file| self.assistantId = file.read}
	...
	def get_validationData
	    begin
		File.open("../keys/sessionValidationData", "rb") {|file| self.sessionValidationData = file.read}
...
class SiriProxy
	def initialize(pluginClasses=[])
		EventMachine.run do
			EventMachine::start_server('0.0.0.0', 446, SiriIPhoneConnection) { |conn|
